...
组件清单是设计和代码之间的契约。组件状态、Token 和代码 API 应使用同一组名称，参考 组件规范（源仓库 UI-UX-docs：04-design-system/components.md）。
...
实现前先检查是否已有 页面模式（源仓库 UI-UX-docs：04-design-system/patterns.md），避免为同一行为新增同义组件。
