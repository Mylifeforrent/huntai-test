...
存放数据模型与 API 规范产出：模块私有逻辑表、ER、字段/约束/分级，以及 API 契约（路径 / 方法 / 请求响应 / 错误码 / SSE）。
...
| [api_spec.md](api_spec.md) | Draft · 本次产出 | OpenAPI 3.1 风格 API 契约：路径/方法/语义 schema/错误码/SSE；不含本阶段表设计与实现代码 |
