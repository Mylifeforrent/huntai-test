...
1. 仓库顶层只允许三类业务目录：`frontend/`、`backend/`、`docs/`；其余只能是仓库级配置文件（`README.md`、`AGENTS.md`、`CLAUDE.md`、`.gitignore`、`.env.example`、`.pre-commit-config.yaml`、`.zcode/`、`.cursor/`、`.mcp.json`、`docker-compose.yml`、`.github/`）。顶层 `docker-compose.yml` 与 `.github/` 仅用于 Stage 12 的编排与 CI 门控，不得写入业务规则；CI 门控命令必须与 §5 一致。
