        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {items.map((item, index) => {
              ...
              return (
                <Link key={id} to={`/projects/${id}/overview`}>
                  <Card className="h-full hover:border-primary/40">
                    <CardHeader>
                      <CardTitle>{String(item.name ?? id)}</CardTitle>
                      <p className="font-mono text-xs text-muted-foreground">
                        Jira: {String(item.jira_project_key ?? "未映射")}
                      </p>
                    </CardHeader>
                    <CardContent>
                      <StatusBadge status={String(item.my_role ?? "")} />
                    </CardContent>
                  </Card>
                </Link>
              );
...
function ProjectDetailView({ projectId }: { projectId: string }) {
  const query = useQuery({
    ...
    queryFn: () =>
      api.get<ResourceEnvelope<Record<string, unknown>>>("API-012", `/api/v1/projects/${projectId}`),
  });
  const data = query.data?.data ?? {};
  const jira = asRecord(data.jira);
  const connectors = Array.isArray(data.connector_health) ? data.connector_health : [];
  const activity = Array.isArray(data.recent_activity) ? data.recent_activity : [];

  return (
    <>
      <PageHeader
        title={typeof data.name === "string" ? data.name : "项目总览"}
        description="工具连接状态 · Jira 映射 · 最近活动"
      />
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Card>
            ...
            <CardContent className="flex flex-col gap-2">
              ...
              {connectors.map((item, index) => {
                ...
                return (
                  <div key={String(row.connector_id ?? index)} className="flex items-center justify-between rounded-md border p-2">
                    <div className="flex items-center gap-2">
                      <span className={healthy ? "size-2 rounded-full bg-success" : "size-2 rounded-full bg-destructive"} />
                      <span className="text-sm">{String(row.name ?? row.type ?? "")}</span>
                      <Badge variant="outline">{String(row.type ?? "")}</Badge>
                    </div>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-1">
              <p className="font-mono text-sm">{String(jira.project_key ?? "未映射")}</p>
          ...
          <Card>
            ...
            <CardContent>
              <StatusBadge status={String(data.my_role ?? "")} />
            </CardContent>
          </Card>
        </div>
        <Card>
              ...
              <Table>
                ...
                <TableBody>
                  {activity.map((item, index) => {
                    const row = asRecord(item);
                    return (
                      <TableRow key={String(row.audit_event_id ?? index)}>
                        <TableCell className="text-xs text-muted-foreground">{String(row.occurred_at ?? "")}</TableCell>
                        <TableCell>{String(row.summary ?? "")}</TableCell>
                        <TableCell className="font-mono text-xs">{String(row.resource_type ?? "")}</TableCell>
                      </TableRow>
                    );
                  })}
                </TableBody>
