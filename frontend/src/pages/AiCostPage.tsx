...
export function AiCostPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      api.get<ResourceEnvelope<AiCostDashboard>>("API-183", "/api/v1/ai/cost-dashboard", {
        from: windowStart,
        to: windowEnd,
        dimension: "workflow",
      }),
  ...
  const costPerWorkflow = totals?.cost_per_workflow ?? {};
    ...
    <>
      ...
      <QueryGate isPending={query.isPending} error={query.error} apis={PAGE_APIS.P21}>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-4">
          ...
          <Stat label="降级率" value={formatNumber(totals?.degrade_rate)} hint="result=degraded" />
        </div>
        <Card>
          <CardHeader>
